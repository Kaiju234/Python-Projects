	...
	def make_tokens(self):
		...
		while self.current_char != None:
			if self.current_char in ' \t':
				...
				tokens.append(self.make_identifer)
				...
				tokens.append(Token(multiplication, pos_start=self.pos))
				self.advance()
			elif self.current_char == '/':
				...
				tokens.append(Token(left_parentheses, pos_start=self.pos))
				self.advance()
			elif self.current_char == ')':
				tokens.append(Token(right_parentheses, pos_start=self.pos))
				self.advance()
			else:
...
class Parser:
	...
	def atom(self):
		...
		elif tok.type == left_parentheses:
			res.register(self.advance())
			expr = res.register(self.expr())
			if res.error: return res
			if self.current_tok.type == right_parentheses:
				res.register(self.advance())
	...
	def term(self):
		return self.bin_op(self.factor, (multiplication, division))
...
class Interpreter:
	...
	def visit_BinOpNode(self, node, context):
		...
		elif node.op_tok.type == multiplication:
